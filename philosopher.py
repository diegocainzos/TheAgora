...
class PhilosopherResponse(BaseModel):
    ...
    philosopher_name: str = Field(
        description="The name of the philosopher speaking (e.g., 'Plato')."
    )
    reaction: str = Field(
        description="A sharp or biting critique of what the previous philosophers said."
    )
...
class Philosopher:
    ...
    def __call__(self, state: AgoraState) -> dict:
        ...
        response = cast(PhilosopherResponse, self.structured_model.invoke(messages))
